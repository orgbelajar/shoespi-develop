...
if (isset($_POST['submit'])) {
   ...
   $name = $_POST['name'];
   $name = filter_var($name, FILTER_SANITIZE_FULL_SPECIAL_CHARS); //Menyaring karakter berbahaya dari input user (prevent XSS attacks)
   $email = $_POST['email'];
   $email = filter_var($email, FILTER_SANITIZE_EMAIL); //Menyaring alamat email
   ...
   if ($old_pass == $empty_pass) {
      ...
      */
      ...
      if ($old_pass === '') {
         ...
         if ($new_pass !== '') {
            // Jika kata sandi baru tidak kosong, hash kata sandi baru sebelum menyimpannya ke database
